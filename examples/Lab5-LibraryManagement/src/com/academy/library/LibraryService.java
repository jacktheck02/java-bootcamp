...
public class LibraryService {
    ...
    public void borrowBook() {
        ...
        if (findMemberById(memberId).equals(null)) {
    ...
    public void returnBook() {
        ...
        for (int i = borrowHistory.size() - 1; i >= 0; i--) {
            if (borrowHistory.get(i).getBookId() == bookId) {
                borrowHistory.get(i).setReturnDate(LocalDate.now());
                break;
            }
        }
    ...
    public void displayBorrowedBooks() {
        ...
        borrowRecords.forEach(
                (bookId, memberId) -> {
                    Book book = findBookById(bookId);
                    Member member = findMemberById(memberId);
                    System.out.printf(
                            "Book: %s (%s) borrowed by %s (%s)%n",
                            bookId,
    ...
    public void sortBooks() {
        ...
        switch (choice) {
            ...
            case "4" ->
                    sortedBooks.sort(
                            Comparator.comparing(Book::getCategory, String.CASE_INSENSITIVE_ORDER));
            default -> {
    ...
    public void displayCategoryInsights() {
        ...
        categoryBookCount.forEach(
                (category, count) -> System.out.printf("%s : %d%n", category, count));
    }
