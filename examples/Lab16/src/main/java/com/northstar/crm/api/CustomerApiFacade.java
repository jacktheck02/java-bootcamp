...
public class CustomerApiFacade {
    ...
    public ApiResult changeStatus(
            String customerId, CustomerStatus newStatus, String correlationId) {
        Customer changedCustomer;
        try {
            changedCustomer = service.changeStatus(customerId, newStatus, correlationId);
        } catch (BusinessException e) {
            return new ApiResult.Fail(handler.fromBusiness(e));
        }

        return new ApiResult.Ok(CustomerMapper.toResponse(changedCustomer));
    }
}
